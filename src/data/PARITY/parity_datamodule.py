...
class PARITY_Dataset(Dataset):
    ...
    def __init__(self, word_length, len, leq = True):
        super(PARITY_Dataset, self).__init__()
...
class PARITY_DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if stage == "fit" or stage is None:
            self.train_set = PARITY_Dataset(word_length=self.word_length, len=self.len, leq=True)
            self.val_set = PARITY_Dataset(word_length=self.word_length, len=self.len, leq=False)

        if stage == "test" or stage is None:
            self.test_set = PARITY_Dataset(word_length=self.word_length, len=self.len, leq=False)
