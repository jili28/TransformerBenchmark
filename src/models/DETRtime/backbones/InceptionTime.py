...
class Inception(ConvNet):
    ...
    def __init__(self, input_shape, output_shape, kernel_size=64, nb_filters=16, batch_size=64,
                 use_residual=True, depth=12, bottleneck_size=16, maxpools = []):
        """
        nb_features: specifies number of channels before the output layer 
        """
        self.bottleneck_size = bottleneck_size
        # these are the 4 concatenated parallel convolutions, width of the inner tensor passed through the network
        ...
        super().__init__(input_shape=input_shape, output_shape=output_shape, kernel_size=kernel_size,
                         nb_filters=nb_filters, batch_size=batch_size, use_residual=use_residual, depth=depth, maxpools = maxpools)
...
class Inception_module(nn.Module):
    ...
    def __init__(self, kernel_size, nb_features, nb_channels, nb_filters, bottleneck_size, depth):
        super().__init__()
        assert kernel_size % 8 == 0
        kernel_size_s = [kernel_size // (2 ** i) for i in range(3)]
        # Define all the layers and modules we need in the forward pass: first the initial convolution and the
    ...
    def forward(self, x):
        ...
        # print(f'x_left1 {x_left1.size()}') 
        # print(f'x_left2 {x_left2.size()}') 
        
        # print(f'x_left3 {x_left3.size()}') 
        # print(f'x_right {x_right.size()}') 
...
if __name__ == '__main__':
    bs, chan, length = 16, 129, 1000
    x = torch.randn(bs, chan, length)
    model = Inception(input_shape=(length, chan), output_shape=None, kernel_size=64, nb_filters=16,
