...
class TransformerModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_pred = y_pred[torch.arange(0, len(x)), lengths]
    ...
    def validation_step(self, batch, barch_idx):
        ...
        return np.array(y_pred.cpu().flatten()), np.array(y.cpu().flatten())
...
def build_causal_encoder(args):
    ...
    model = TransformerModel(
        ...
        hidden_size=args['hidden_size'], num_layers=args['num_layers'], vocab_size=args['vocab_size'],
        bracket_num=args['vocab_size'] / 2, embedding_type=args['embedding_type'])
